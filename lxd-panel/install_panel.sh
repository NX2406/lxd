...
# 生成随机访问路径（提高安全性）
RANDOM_PATH="/panel-$(cat /dev/urandom | tr -dc 'a-z0-9' | fold -w 8 | head -n 1)"
echo "生成随机访问路径: $RANDOM_PATH"
...
if command -v apt-get > /dev/null 2>&1; then
    ...
    echo "安装依赖包: python3 python3-pip python3-venv nginx jq bc curl expect..."
    apt-get install -y python3 python3-pip python3-venv nginx jq bc curl expect
    ...
    echo "安装依赖包..."
    yum install -y python3 python3-pip nginx jq bc curl expect
...
# 确保pip是最新的
echo "更新pip..."
python3 -m pip install --upgrade pip --break-system-packages --quiet 2>/dev/null || true
...
# 显示安装过程，方便调试
echo "安装Python包..."
# Ubuntu 24.04需要--break-system-packages标志
if python3 -m pip install -r requirements.txt --break-system-packages --quiet 2>/dev/null; then
    echo -e "${GREEN}Python依赖安装成功${NC}"
else
    echo -e "${YELLOW}尝试使用清华源重新安装...${NC}"
    python3 -m pip install -r requirements.txt --break-system-packages -i https://pypi.tuna.tsinghua.edu.cn/simple
fi
...
# 使用模板生成配置文件
if [ -f "$SCRIPT_DIR/nginx/lxd-panel.conf.template" ]; then
    TEMPLATE_FILE="$SCRIPT_DIR/nginx/lxd-panel.conf.template"
elif [ -f "$INSTALL_DIR/nginx/lxd-panel.conf.template" ]; then
    TEMPLATE_FILE="$INSTALL_DIR/nginx/lxd-panel.conf.template"
else
    echo -e "${RED}找不到nginx配置模板${NC}"
    exit 1
fi

# 替换占位符生成最终配置
sed "s|PLACEHOLDER_PATH|$RANDOM_PATH|g" "$TEMPLATE_FILE" > /etc/nginx/sites-available/lxd-panel.conf

# 保存随机路径到配置文件
echo "$RANDOM_PATH" > /opt/lxd-panel/ACCESS_PATH.txt
chmod 600 /opt/lxd-panel/ACCESS_PATH.txt

echo "访问路径已保存到: /opt/lxd-panel/ACCESS_PATH.txt"
...
echo "==================================================="
echo ""
echo -e "访问地址: ${GREEN}http://$SERVER_IP$RANDOM_PATH${NC}"
echo -e "随机路径: ${YELLOW}$RANDOM_PATH${NC}"
echo -e "默认用户名: ${YELLOW}admin${NC}"
echo -e "默认密码: ${YELLOW}admin${NC}"
echo ""
echo -e "${YELLOW}重要提示:${NC}"
echo "1. 请立即修改默认密码"
echo "2. 请妥善保管访问路径,路径已保存到: /opt/lxd-panel/ACCESS_PATH.txt"
echo "3. 直接访问IP地址将返回404,只有通过随机路径才能访问"
echo "4. 建议配置HTTPS证书以提高安全性"
echo "5. 查看后端日志: journalctl -u lxd-panel -f"
echo "6. 查看Nginx日志: tail -f /var/log/nginx/access.log"
echo ""
echo "==================================================="
echo ""
