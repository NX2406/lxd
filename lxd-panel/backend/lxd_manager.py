...
class LXDManager:
    ...
    def _parse_description(self, desc: str) -> Tuple[Optional[int], Optional[str], Optional[str]]:
        """解析description字段
        支持格式: 
        1. name ssh_port password nat_start nat_end
        2. 直接的文本描述（返回None）
        """
        ...
        try:
            ...
            nat_ports = None
            if len(parts) >= 5:
                if parts[3].isdigit() and parts[4].isdigit() and int(parts[3]) > 0:
                    nat_ports = f"{parts[3]}-{parts[4]}"
            
            return ssh_port, nat_ports, password
        except Exception as e:
            print(f"解析description失败: {e}")
            return None, None, None
    ...
    def get_host_info(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "cpu_usage": cpu_percent,  # 改为cpu_usage以匹配前端
                "memory_total": memory.total // (1024 * 1024),  # MB
                "memory_used": memory.used // (1024 * 1024),
                "memory_percent": memory.percent,
                "disk_total": disk.total,  # 字节
                "disk_used": disk.used,  # 字节
    ...
    def create_instance(self, name: str, os_type: str, os_version: str,
                       cpu: float, memory: int, disk: int,
                       ssh_port: int, nat_start: int, nat_end: int) -> bool:
        """创建新实例
        
        Args:
            name: 实例名称
            os_type: 操作系统类型 (debian, ubuntu, centos等)
            os_version: 系统版本
            cpu: CPU核数
            memory: 内存大小(MB)
            disk: 硬盘大小(GB)
        ...
        try:
            ...
            # 确定镜像源
            image_alias = f"{os_type}/{os_version}"
            
            # 配置资源限制
            config = {
                'limits.memory': f'{memory}MiB',
            ...
            # CPU配置
            if cpu == int(cpu):  # 整数核心
                config['limits.cpu'] = str(int(cpu))
            else:  # 小数核心
                cpu_count = int(cpu + 0.99)
                cpu_allowance = int((cpu / cpu_count) * 100)
                config['limits.cpu'] = str(cpu_count)
                config['limits.cpu.allowance'] = f'{cpu_allowance}%'
            ...
            # 生成随机密码
            import hashlib
            password = hashlib.md5(str(time.time()).encode()).hexdigest()[:9]
            ...
            # 设置description保存信息
            nat_start_str = nat_start if nat_start > 0 else 0
            nat_end_str = nat_end if nat_end > 0 else 0
            description = f"{name} {ssh_port} {password} {nat_start_str} {nat_end_str}"
            container.description = description
            container.save()
            
            print(f"实例 {name} 创建成功，密码: {password}")
            return True
