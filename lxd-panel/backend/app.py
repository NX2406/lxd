...
@app.post("/api/instances", response_model=OperationResponse, tags=["实例管理"])
async def create_instance(request: CreateInstanceRequest):
    """创建新实例"""
    try:
        # 调用原始的panel.txt脚本创建逻辑
        # 这里简化处理，只创建基本容器
        success = lxd_manager.create_instance(
            name=request.name,
            ...
            nat_start=request.nat_start,
            nat_end=request.nat_end
        )
        return OperationResponse(
            success=success,
            message=f"实例 {request.name} {'创建成功' if success else '创建失败'}"
        )
    except Exception as e:
...
@app.post("/api/instances/batch/start", response_model=OperationResponse, tags=["批量操作"])
async def batch_start_instances():
    """批量启动所有实例"""
    try:
        instances = lxd_manager.get_all_instances()
        count = 0
        for inst in instances:
            if inst.status != 'Running':
                if lxd_manager.start_instance(inst.name):
                    count += 1
        return OperationResponse(
            success=True,
            message=f"成功启动 {count} 个实例"
        )
    except Exception as e:
        return OperationResponse(success=False, message=str(e))


@app.post("/api/instances/batch/stop", response_model=OperationResponse, tags=["批量操作"])
async def batch_stop_instances():
    """批量停止所有实例"""
    try:
        instances = lxd_manager.get_all_instances()
        count = 0
        for inst in instances:
            if inst.status == 'Running':
                if lxd_manager.stop_instance(inst.name):
                    count += 1
        return OperationResponse(
            success=True,
            message=f"成功停止 {count} 个实例"
        )
    except Exception as e:
        return OperationResponse(success=False, message=str(e))
